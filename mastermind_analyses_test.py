				...
				if n == len(noeud): # code complet			
					...
					for t,s in states.items():
						...
						if mm_temp.get_states()[len(mm_temp.get_states())][1] > s[1] or mm_temp.get_states()[len(mm_temp.get_states())][2] > s[2]:
							compatible = False
							break
						elif mm_temp.get_states()[len(mm_temp.get_states())][1] == s[1] or mm_temp.get_states()[len(mm_temp.get_states())][2] == s[2]:
							for caractere in code_temp:
								if caractere in D_bis:
									D_bis.remove(caractere)
							if len(D_bis) > (n-len(i)):
								compatible = False
								break				
					if compatible == False:
						continue
