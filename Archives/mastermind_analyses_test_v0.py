...
def RAC_forward_checking_doublons_LasVegas(i,nbreVar,D,n,states,nbre_occurences):
	'''
	i : instanciation courante
	V : liste de variables non-instanciées dans i
	D : domaines des variables
	...
	if nbreVar == 0:
		...
		nbreVar -= 1
		D_LasVegas = [lv for lv in range(len(D))]
		print(D_LasVegas)
		for lv in range(len(D_LasVegas)):
			j = random.choice(D_LasVegas)
			D_LasVegas.remove(j)
			print(j)
			print(D_LasVegas)
			nbre_occurences_bis = nbre_occurences.copy()
			...
			if n == len(noeud):
				...
				if cd: # si le code est compatible, on le prend
					return i_dico,nbre_noeuds
			res,nbre_noeuds_temp = RAC_forward_checking_doublons_LasVegas(i+list(v),nbreVar,D_bis,n,states,nbre_occurences_bis)
			nbre_noeuds += nbre_noeuds_temp
			if len(res) == n and compatibilite_doublons(n,states,res):
				i = res
				break
...
def run(n=4,joueur=0,code_secret=['0','1','2','3'],premiere_tentative={0: '0', 1: '1', 2: '2', 3: '3'},strategie_algo_genetique=0,maxsize=10,maxgen=105,popsize=50,CXPB=0.6,MUTPB=0.4):
	...
	if (joueur == 0):
		...
		while(True):
			...
			elif (joueur == 7): # bonus : A4 version Las Vegas
				res = {}
				if mastermind.get_nb_tentatives() == 0:
					res = premiere_tentative
				else:
					...
					states = mastermind.get_states()
					nbre_occurences = [0] * mastermind.get_p()
					res,nbre_noeuds_temp = RAC_forward_checking_doublons_LasVegas(i,nbreVar,D,n,states,nbre_occurences)
					nbre_noeuds += nbre_noeuds_temp
					#print("Nombre de noeuds (en comptant les précédents) :",nbre_noeuds)
					input()
			...
			elif (joueur == 8): # bonus : A5 version Las Vegas
				res = {}
				if mastermind.get_nb_tentatives() == 0:
					res = premiere_tentative
				else:
			...
			elif (joueur == 9): # algorithme génétique
	...
	for n in tailles_n:
		...
		for i in range(nbre_instances):
			...
			for i in range(len(premiere_tentative_liste)):
				premiere_tentative[i] = premiere_tentative_liste[i]
			start_time = time.time()
			run(n=n,joueur=9,reponse=code_secret,premiere_tentative=premiere_tentative,strategie_algo_genetique=0,maxsize=maxsize,maxgen=maxgen,popsize=popsize,CXPB=cxpb,MUTPB=mutpb)
			temps_AG0 += time.time() - start_time
			start_time = time.time()
			run(n=n,joueur=9,reponse=code_secret,premiere_tentative=premiere_tentative,strategie_algo_genetique=1,maxsize=maxsize,maxgen=maxgen,popsize=popsize,CXPB=cxpb,MUTPB=mutpb)
			temps_AG1 += time.time() - start_time
			start_time = time.time()
			run(n=n,joueur=9,reponse=code_secret,premiere_tentative=premiere_tentative,strategie_algo_genetique=2,maxsize=maxsize,maxgen=maxgen,popsize=popsize,CXPB=cxpb,MUTPB=mutpb)
			temps_AG2 += time.time() - start_time
			start_time = time.time()
			run(n=n,joueur=9,reponse=code_secret,premiere_tentative=premiere_tentative,strategie_algo_genetique=3,maxsize=maxsize,maxgen=maxgen,popsize=popsize,CXPB=cxpb,MUTPB=mutpb)
			temps_AG3 += time.time() - start_time
			start_time = time.time()
			run(n=n,joueur=9,reponse=code_secret,premiere_tentative=premiere_tentative,strategie_algo_genetique=4,maxsize=maxsize,maxgen=maxgen,popsize=popsize,CXPB=cxpb,MUTPB=mutpb)
			temps_AG4 += time.time() - start_time
			start_time = time.time()
			run(n=n,joueur=9,reponse=code_secret,premiere_tentative=premiere_tentative,strategie_algo_genetique=5,maxsize=maxsize,maxgen=maxgen,popsize=popsize,CXPB=cxpb,MUTPB=mutpb)
			temps_AG5 += time.time() - start_time
			start_time = time.time()
			run(n=n,joueur=9,reponse=code_secret,premiere_tentative=premiere_tentative,strategie_algo_genetique=6,maxsize=maxsize,maxgen=maxgen,popsize=popsize,CXPB=cxpb,MUTPB=mutpb)
...
def graphe2_2_evolution_nbre_essais(tailles_n,nbre_instances,maxsize,maxgen,popsize,cxpb,mutpb):
	...
	for n in tailles_n:
		...
		for i in range(nbre_instances):
			...
			for i in range(len(premiere_tentative_liste)):
				premiere_tentative[i] = premiere_tentative_liste[i]
			nb_tentativesAG0,nb_noeudsAG0 = run(n=n,joueur=9,reponse=code_secret,premiere_tentative=premiere_tentative,strategie_algo_genetique=0,maxsize=maxsize,maxgen=maxgen,popsize=popsize,CXPB=cxpb,MUTPB=mutpb)
			nb_tentativesAG1,nb_noeudsAG1 = run(n=n,joueur=9,reponse=code_secret,premiere_tentative=premiere_tentative,strategie_algo_genetique=1,maxsize=maxsize,maxgen=maxgen,popsize=popsize,CXPB=cxpb,MUTPB=mutpb)
			nb_tentativesAG2,nb_noeudsAG2 = run(n=n,joueur=9,reponse=code_secret,premiere_tentative=premiere_tentative,strategie_algo_genetique=2,maxsize=maxsize,maxgen=maxgen,popsize=popsize,CXPB=cxpb,MUTPB=mutpb)
			nb_tentativesAG3,nb_noeudsAG3 = run(n=n,joueur=9,reponse=code_secret,premiere_tentative=premiere_tentative,strategie_algo_genetique=3,maxsize=maxsize,maxgen=maxgen,popsize=popsize,CXPB=cxpb,MUTPB=mutpb)
			nb_tentativesAG4,nb_noeudsAG4 = run(n=n,joueur=9,reponse=code_secret,premiere_tentative=premiere_tentative,strategie_algo_genetique=4,maxsize=maxsize,maxgen=maxgen,popsize=popsize,CXPB=cxpb,MUTPB=mutpb)
			nb_tentativesAG5,nb_noeudsAG5 = run(n=n,joueur=9,reponse=code_secret,premiere_tentative=premiere_tentative,strategie_algo_genetique=5,maxsize=maxsize,maxgen=maxgen,popsize=popsize,CXPB=cxpb,MUTPB=mutpb)
			nb_tentativesAG6,nb_noeudsAG6 = run(n=n,joueur=9,reponse=code_secret,premiere_tentative=premiere_tentative,strategie_algo_genetique=6,maxsize=maxsize,maxgen=maxgen,popsize=popsize,CXPB=cxpb,MUTPB=mutpb)
...
def graphe2_3_evolution_temps(tailles_n,nbre_instances,meilleure_strategie_partie_1,strategie_algo_genetique,maxsize,maxgen,popsize,cxpb,mutpb):
	...
	for n in tailles_n:
		...
		for i in range(nbre_instances):
			...
			start_time = time.time()
			run(n=n,joueur=9,reponse=code_secret,premiere_tentative=premiere_tentative,strategie_algo_genetique=strategie_algo_genetique,maxsize=maxsize,maxgen=maxgen,popsize=popsize,CXPB=cxpb,MUTPB=mutpb) 
...
def graphe2_3_evolution_nbre_essais(tailles_n,nbre_instances,meilleure_strategie_partie_1,strategie_algo_genetique,maxsize,maxgen,popsize,cxpb,mutpb):
	...
	for n in tailles_n:
		...
		for i in range(nbre_instances):
			...
			nb_tentativesAG,nb_noeudsAG = run(n=n,joueur=9,reponse=code_secret,premiere_tentative=premiere_tentative,strategie_algo_genetique=strategie_algo_genetique,maxsize=maxsize,maxgen=maxgen,popsize=popsize,CXPB=cxpb,MUTPB=mutpb) 
