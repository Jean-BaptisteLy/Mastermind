	...
	if len(V) == 0:
		...
		for v in D[xk]:
			...
			if len(noeud) == len(set(noeud)): # si c'est localement consistant : caractères uniques
				...
				res,nbre_noeuds_temp = RAC(i+list(v),V_temp,D,n,states)
		...
		for v in D[xk]:
			...
			if len(noeud) == len(set(noeud)): # si c'est localement consistant : caractères uniques
				if n == len(noeud):
					...
					if compatibilite(n,states,i_dico): # si le code est compatible, on le prend
						return i_dico,nbre_noeuds
				res,nbre_noeuds_temp = RAC_forward_checking(i+list(v),V_temp,D_bis,n,states)
				nbre_noeuds += nbre_noeuds_temp
				if len(res) == n and compatibilite(n,states,res):
					i = res
					break
		...
		for j in range(len(D[xk])):
			...
			if n == len(noeud):
				...
				if cd: # si le code est compatible, on le prend
					return i_dico,nbre_noeuds
			res,nbre_noeuds_temp = RAC_forward_checking_doublons(i+list(v),V_temp,D_bis,n,states,nbre_occurences_bis)
			nbre_noeuds += nbre_noeuds_temp
		...
		for v in D[xk]:
			...
			if len(noeud) == len(set(noeud)): # si c'est localement consistant : caractères uniques
				...
				if n == len(noeud): # code complet			
					...
					if compatible == False:
						continue
				res,nbre_noeuds_temp = RAC_forward_checking_ameliore(i+list(v),V_temp,D_bis,n,states)
				nbre_noeuds += nbre_noeuds_temp
				if len(res) == n and compatibilite(n,states,res):
					i = res
					break
		...
		while(True):
			...
			#################################################################################################################################################################################################
				...
				if mastermind.get_nb_tentatives() == 0:
					...
					while True:
						...
						nbre_noeuds += 1
						#print("noeud :",res) # permet de voir les noeuds de l'arbre
			...
			#################################################################################################################################################################################################
				...
				if mastermind.get_nb_tentatives() == 0:
					...
					res,nbre_noeuds_temp = RAC(i,V,D,n,states)
					nbre_noeuds += nbre_noeuds_temp
			...
			#################################################################################################################################################################################################
				...
				if mastermind.get_nb_tentatives() == 0:
					...
					D = deepcopy(D_RAC)
					states = mastermind.get_states()
					res,nbre_noeuds_temp = RAC_forward_checking(i,V,D,n,states)
					nbre_noeuds += nbre_noeuds_temp
					#print("Nombre de noeuds (en comptant les précédents) :",nbre_noeuds)
					#input()
			...
			#################################################################################################################################################################################################
				...
				if mastermind.get_nb_tentatives() == 0:
					...
					res,nbre_noeuds_temp = RAC_forward_checking_doublons(i,V,D,n,states,nbre_occurences)
					nbre_noeuds += nbre_noeuds_temp
					#print("Nombre de noeuds (en comptant les précédents) :",nbre_noeuds)
					#input()
			...
			#################################################################################################################################################################################################
				...
				if mastermind.get_nb_tentatives() == 0:
					...
					D = deepcopy(D_RAC)
					states = mastermind.get_states()
					res,nbre_noeuds_temp = RAC_forward_checking_ameliore(i,V,D,n,states)
					nbre_noeuds += nbre_noeuds_temp
					#print("Nombre de noeuds (en comptant les précédents) :",nbre_noeuds)
					#input()
